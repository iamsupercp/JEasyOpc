...
uses
  Classes, Windows, ActiveX, OPCCOMN, dialogs;
...
begin
  try
    if Length(FComputerName) = 0
    then PUnk := CreateComObject(CLSID_OPCServerList)
    else PUnk := CreateRemoteComObject(FComputerName, CLSID_OPCServerList);
  except
    PUnk := nil;
  end;
